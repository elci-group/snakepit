use anyhow::Result;
use clap::Parser;
use snakegg::native::style::{blue, bold, cyan, dim, green, magenta, red, yellow};
use std::path::Path;
...
use cli::Cli;
use config::{ProjectConfig, SnakepitConfig};
use daemon::{DaemonConfig, DaemonManager};
use dashboard::DashboardServer;
use dependency::{Dependency, ProjectDependencies};
use handler::SnakepitHandler;
use installer::{InstallerBackend, PackageInstaller};
use resolver::DependencyResolver;
use venv::{VenvBackend, VirtualEnvironmentManager};
    ...
    match cli.command {
        ...
        cli::Commands::Fix { command } => {
            if command.is_empty() {
                println!(
                    "{}",
                    yellow("Please provide a command to fix, e.g., 'snakepit fix -- adk'")
                );
                return Ok(());
            }
            ...
            println!(
                "{}",
                cyan(format!("🔧 Running command to diagnose: {}", cmd_str))
            );
                    ...
                    Ok(output) => {
                        ...
                        if let Some(lib_name) = sys_detector.extract_library_from_error(&stderr) {
                            println!(
                                "{}",
                                yellow(format!(
                                    "🔧 SYSTEM: Detected missing library: {}",
                                    lib_name
                                ))
                            );
                            ...
                            if let Some(lib) = sys_detector.find_package(&lib_name) {
                                println!(
                                    "{}",
                                    green(format!(
                                        "💡 SUGGESTION: Install system package '{}'",
                                        lib.package_name
                                    ))
                                );
                                ...
                                if let Some(cmd) = sys_detector.get_install_command(&lib) {
                                    ...
                                    println!(
                                        "{}",
                                        yellow(
                                            "⚠️  Could not determine install command for your OS."
                                        )
                                    );
                                    break;
                                }
                                ...
                                println!(
                                    "{}",
                                    yellow(format!("⚠️  Unknown system library: {}", lib_name))
                                );
                        ...
                        // If not a system library error, check for a missing Python module
                        if let Some(module) = missing_python_module(&stderr) {
                            println!(
                                "{}",
                                magenta(format!("🐍 Missing Python module detected: {}", module))
                            );
                            println!(
                                "{}",
                                green(format!(
                                    "💡 Suggestion: Install '{}' to fix the error.",
                                    module
                                ))
                            );
                            ...
                            if handler.handle_package(&module, None, None).await? {
                            ...
                            println!("{}", yellow("❌ Command failed. No missing system library or Python module detected."));
            ...
            match action {
                ...
                cli::FangsAction::Cache {
                    ...
                    fangs.add_modification(ModificationRule {
                        target_module: module.clone(),
                        target_function: function.clone(),
                        modification_type: ModificationType::InjectCache { ttl_seconds: ttl },
                    });
                    fangs.apply_modifications(&fork_dir)?;
                }
                cli::FangsAction::Mock {
                    module,
                    function,
                    value,
                } => {
                ...
                cli::FangsAction::Custom {
                    ...
                    fangs.add_modification(ModificationRule {
                        target_module: module.clone(),
                        target_function: function.clone(),
                        modification_type: ModificationType::CustomCode { code: code.clone() },
                    });
                    fangs.apply_modifications(&fork_dir)?;
                }
        ...
        cli::Commands::Ui { port } => {
            ...
            let project_name = project_deps.project_name.clone().unwrap_or_else(|| {
                std::env::current_dir()
                    .ok()
...
async fn uninstall_package(package: &str, _config: &SnakepitConfig) -> Result<()> {
    ...
    if report.risk_score > 50 {
        println!(
            "{}",
            yellow(format!(
                "⚠️  High risk detected! Risk Score: {}",
                report.risk_score
            ))
        );
    ...
    match uninstaller.create_snapshot(package).await {
        ...
        Err(e) => println!(
            "{}",
            yellow(format!("⚠️  Failed to create snapshot: {}", e))
        ),
    }
...
async fn list_packages(_config: &SnakepitConfig) -> Result<()> {
    ...
    let installer = PackageInstaller::new().with_venv(sp_path.clone());
    ...
    if packages.is_empty() {
        println!(
            "{}",
            yellow("📭 No packages installed in snakepit environment.")
        );
        return Ok(());
    }

    let max_size = packages
        .iter()
        .map(|p| p.size_bytes)
        .max()
        .unwrap_or(1)
    ...
    println!(
        "  {}",
        dim("─".repeat(max_name_len + max_ver_len + bar_width + 14))
    );
...
/// Extract a module name from Python's `ModuleNotFoundError: No module named 'x'` output.
fn missing_python_module(stderr: &str) -> Option<String> {
    let re = regex::Regex::new(r#"No module named ['"]([A-Za-z0-9_.]+)['"]"#).ok()?;
...
async fn search_packages(query: &str, config: &SnakepitConfig) -> Result<()> {
    ...
    if results.is_empty() {
        println!(
            "{}",
            yellow(format!("No packages found matching '{}'", query))
        );
    } else {
...
async fn init_project(name: Option<&str>, config: &SnakepitConfig) -> Result<()> {
    ...
    println!(
        "{}",
        blue(format!("Initializing project '{}'...", project_name))
    );
    ...
    if let Some(venv_backend) = &config.default_venv_backend {
        let venv_manager =
            VirtualEnvironmentManager::new().with_backend(match venv_backend.as_str() {
        ...
        let venv_path = venv_manager
            .create_venv(project_name, config.python_version.as_deref())
            .await?;
        println!(
            "{}",
            green(format!(
...
async fn handle_venv_command(command: cli::VenvCommands, config: &SnakepitConfig) -> Result<()> {
    ...
    match command {
        cli::VenvCommands::Create {
            name,
            python_version,
        } => {
            let venv_path = venv_manager
                .create_venv(&name, python_version.as_deref())
                .await?;
            println!(
                "{}",
                green(format!(
        ...
        cli::VenvCommands::Delete { name } => {
            ...
            println!(
                "{}",
                green(format!("✓ Virtual environment '{}' deleted", name))
            );
        }
...
async fn update_project_dependencies(
    _dependency: &Dependency,
    _config: &SnakepitConfig,
) -> Result<()> {
...
async fn handle_daemon_command(
    command: cli::DaemonCommands,
    config: &SnakepitConfig,
) -> Result<()> {
    let daemon_manager = DaemonManager::new();

    match command {
        cli::DaemonCommands::Start {
            daemon,
            config: _config_path,
        } => {
            if daemon {
        ...
        cli::DaemonCommands::Status => {
            ...
            println!(
                "  Check Interval: {}s",
                status.config.check_interval.as_secs()
            );
        }
...
async fn handle_daemon_config_command(
    command: cli::DaemonConfigCommands,
...
async fn handle_run_command(command: Vec<String>) -> Result<()> {
    ...
    if command.is_empty() {
        ...
        println!(
            "{}",
            dim("  snakepit run -- python3 test_censoring_methods.py")
        );
        return Ok(());
    }
    ...
    if !env.exists() {
        ...
        println!(
            "{}",
            yellow("Or install a package: snakepit install <package>")
        );
        return Ok(());
    }
    ...
    println!(
        "{}",
        dim(format!(
            "🐍 Running in snakepit environment: {}",
            env.get_path().display()
        ))
    );
...
async fn handle_env_command(command: cli::EnvCommands) -> Result<()> {
    ...
    match command {
        ...
        cli::EnvCommands::Init { python_version } => {
            ...
            println!(
                "{}",
                dim("   Install packages with: snakepit install <package>")
            );
            println!(
                "{}",
                dim("   Run scripts with: snakepit run -- python your_script.py")
            );
        }
        ...
        cli::EnvCommands::Path => {
            if env.exists() {
                ...
                return Err(anyhow::anyhow!(
                    "No snakepit environment found. Run 'snakepit env init' first."
                ));
            }
        }
    }
