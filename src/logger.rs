...
impl GitLogger {
    pub fn new(log_dir: PathBuf, remote_url: Option<String>) -> Self {
        ...
        if !log_dir.join(".git").exists() {
            ...
            let _ = Command::new("git")
                .args(["config", "user.name", "Snakepit Logger"])
                .current_dir(&log_dir)
                .output();
            let _ = Command::new("git")
                .args(["config", "user.email", "logger@snakepit.local"])
                .current_dir(&log_dir)
                .output();
        }
    ...
    pub async fn sync(&mut self) -> Result<()> {
        ...
        if std::env::var("SNAKEPIT_ENABLE_LOG_SYNC").as_deref() != Ok("1") {
            return Ok(());
        }
