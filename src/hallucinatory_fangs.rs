...
impl HallucinatoryFangs {
    pub fn new() -> Result<Self> {
        if std::env::var("SNAKEPIT_ENABLE_FANGS").as_deref() != Ok("1") {
            return Err(anyhow::anyhow!(
                "Hallucinatory Fangs is disabled by default. Set SNAKEPIT_ENABLE_FANGS=1 to modify forked code."
            ));
        }
    ...
    fn find_module(&self, module_name: &str) -> Result<PathBuf> {
        Self::validate_module_name(module_name)?;
        // Pass the module name as data rather than interpolating it into Python source.
        let output = std::process::Command::new("python3")
            .arg("-c")
            .arg("import importlib, os, sys; print(os.path.dirname(importlib.import_module(sys.argv[1]).__file__))")
            .arg(module_name)
            .output()?;
    ...
    fn validate_module_name(module_name: &str) -> Result<()> {
        let valid = !module_name.is_empty()
            && module_name.len() <= 214
            && module_name.split('.').all(|part| {
        ...
        if valid {
            ...
            Err(anyhow::anyhow!("Invalid Python module name"))
        }
    }
    ...
    fn find_module_file(&self, fork_dir: &PathBuf, module_name: &str) -> Result<PathBuf> {
        ...
        Err(anyhow::anyhow!(
            "Could not find module file for '{}'",
            module_name
        ))
    }
