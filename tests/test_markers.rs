use snakepit::markers::{EnvironmentMarker, TargetEnvironment};
...
fn test_marker_evaluation() {
    ...
    let marker = EnvironmentMarker {
        raw: "sys_platform == 'linux'".to_string(),
    };
    assert!(marker.evaluate(&env));

    let marker = EnvironmentMarker {
        raw: "sys_platform == 'win32'".to_string(),
    };
    assert!(!marker.evaluate(&env));

    let marker = EnvironmentMarker {
        raw: "python_version >= '3.8'".to_string(),
    };
    assert!(marker.evaluate(&env));

    // Complex markers are not fully supported by the simplified parser yet,
    // but we can test the basic extraction logic
    let _marker = EnvironmentMarker {
        raw: "sys_platform == 'linux' and python_version >= '3.8'".to_string(),
    };
